...
public class ReceiptImageController {
    ...
    public ReceiptSuggestionResponse parseReceipt(@NotEmpty String base64EncodedImage) throws Exception {
        ...
        try (ImageAnnotatorClient client = ImageAnnotatorClient.create()) {
            ...
            //res = res.getFullTextAnnotation().toString()
            String resText = res.getFullTextAnnotation().getText();
            out.println(res);
            ...
            // regex string for word
            Pattern wPattern = Pattern.compile("\\w+");
            Matcher wMatcher = wPattern.matcher(resText);

            // regex string for decimal
            Pattern dPattern = Pattern.compile("\\d+\\.\\d+");
            Matcher dMatcher = dPattern.matcher(resText);

            if(wMatcher.find())
                merchantName = wMatcher.group();
            String val = "";
            while(dMatcher.find())
                val = dMatcher.group();
